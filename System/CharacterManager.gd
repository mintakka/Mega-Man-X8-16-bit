...
const valid_characters := ["X", "Zero", "Axl"]

# CharacterManager is the sole authority for the selected native scene and its
# variants. Savefile embeds this state per slot while char_data remains readable
# for unmodified Zashiko saves.
...
func apply_save_state(save_data: Dictionary) -> void:
	var selected = save_data.get("player_character", "X")
	if typeof(selected) == TYPE_INT or typeof(selected) == TYPE_REAL:
		# The retired CharacterRoster used ZERO=0, X=1; accept Axl=2 for
		# forward-compatible experimental saves.
		selected = {0: "Zero", 1: "X", 2: "Axl"}.get(int(selected), "X")
	if not selected in valid_characters:
		selected = "X"
	player_character = selected
	player_count = int(save_data.get("player_count", {"X": 1, "Axl": 2, "Zero": 3}.get(selected, 1)))
	player_count = int(clamp(player_count, min_player_count, max_player_count))
	...
	ultimate_x_armor = bool(save_data.get("ultimate_x_armor", false)) and "ultima_head" in GameManager.collectibles and evaluate_ultimate_armor_state()
	black_zero_armor = bool(save_data.get("black_zero_armor", false)) and "black_zero_armor" in GameManager.collectibles
	white_axl_armor = bool(save_data.get("white_axl_armor", false)) and "white_axl_armor" in GameManager.collectibles
	update_game_mode()
	check_for_deactivated_skills_Zero()

func _save() -> void :
	var save_data = export_save_state()
...
func get_player_character_object() -> PackedScene:
