...
func charged_shot_release(_charge_level):
	if not character.has_control():
		...
		return
	if current_weapon.name == "NovaStrike":
		return
...
func _StartCondition() -> bool:
	if current_weapon and character.has_control():
		# Charge emits the tier-2 projectile for this press. Do not also spawn
		# the ordinary lemon; fire_charged() uses ExecuteOnce and bypasses this
		# input gate when the deferred charged signal arrives.
		if GameManager.cheat_fast_max_charge and CharacterManager.player_character == "X" and current_weapon_is_buster() and action_just_pressed():
			return false
