...
var mid_charge: bool
var charging: = false
var fast_charge_shot_fired := false
...
func _ready() -> void :
	...
	audio2 = get_node("audioStreamPlayer2")
...
func _Setup():
	...
	fast_charge_shot_fired = false
	if is_fast_charge_buster():
		# Exactly tier 2: the normal magenta full-charge shot. Continued hold
		# still advances into X's native tier-3 armor blast.
		charged_time = level_3_charge * (1 - charge_time_reduction)
		call_deferred("fire_fast_charge_tap")

func is_fast_charge_buster() -> bool:
	if not GameManager.cheat_fast_max_charge or CharacterManager.player_character != "X":
		return false
	if not arm_cannon.current_weapon:
		return false
	return arm_cannon.current_weapon_is_buster()

func fire_fast_charge_tap() -> void:
	if executing and character.listening_to_inputs and is_fast_charge_buster() and not fast_charge_shot_fired:
		fast_charge_shot_fired = true
		Event.emit_signal("charged_shot_release", 2)
...
func setup_charge_fadeout() -> void :
	...
	if Configurations.get("ChargeFadeOut"):
		...
		tween.add_attribute("volume_db", - 80.0, 65.0, audio)
		tween.add_attribute("volume_db", - 80.0, 65.0, audio2)
