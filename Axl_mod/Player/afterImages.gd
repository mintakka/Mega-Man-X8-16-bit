...
export  var frequency: float = 0.075
...
export  var afterimage_hitbox: PackedScene = preload("res://Axl_mod/Player/Projectiles/BossWeapons/BlackArrow/BlackArrowSpecial_Hitbox.tscn")
...
func afterimage_emit() -> void :
	if active:
		...
		var hitbox = afterimage_hitbox.instance()
		hitbox.timer = duration
		afterimage.add_child(hitbox)
